 ...
 * @FilePath: \lixue-psych-uniapp\pages\index\quan\components\lawVotePopup.vue
 * @Description: 法律相关问题投票弹窗
-->
<template>
	...
	export default {
		...
		methods: {
			...
			async getVoteList() {
				...
				const lawAreas = this.getLeafNodes(res.data[1])
				lawAreas.forEach((item) => {
					this.$set(item, 'active', false)
				})
				this.lawAreas = lawAreas
				this.currentCategoryId = lawAreas[0].id
				try {
