	 ...
	 * @FilePath: \lixue-psych-uniapp\pages\index\quan\zznx.vue
	 * @Description: quan首页(郑在暖心篇)
	-->
	<template>
		<view class="rpage" :style="'min-height:' + windowHeight + 'px;'">
			...
			<view class="top-bg">
				...
				<view class="banner">
					<image src="/static/images/banner-zznx.png" class="img" mode="widthFix" />
				</view>
			</view>
			<view class="care-box">
				<view class="care-title">大数据统计—市民关心法律问题</view>
				<view class="main">
					<RectTreeMap :treeData="cardData" :windowWidth="windowWidth" type='zznx' :isPercentageShow="false"/>
				</view>
			</view>
			<view class="bottom-placeholder"></view>
			<ContactUs style="padding:24rpx 32rpx;"></ContactUs>
			<Partners :data="['郑州大数据发展有限公司', '郑州立雪智能科技研究院']" />
			...
			methods: {
				...
				getNav(list) {
					getCategory().then((res) => {
						const list = treeToList(res.data[2].children)
						this.navList = list;
				...
				navItemClick(item, indx) {
					getCartCards({
						...
						this.cardData = res.data.sort((a, b) => b.orderCount - a.orderCount).map((item, index) => ({
							...item,
							color: item.color ?? colorList3[index],
